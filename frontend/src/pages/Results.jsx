...
export default function Results({ results, addToCompare }) {
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', padding: '5rem 2rem', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'fixed', top: 0, left: '50%', transform: 'translateX(-50%)', width: '700px', height: '400px', background: 'radial-gradient(ellipse at center top, rgba(240,240,240,.08) 0%, transparent 70%)', pointerEvents: 'none' }} />

      <div style={{ maxWidth: '960px', margin: '0 auto', position: 'relative' }}>

        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: '5rem' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '.5rem', background: 'rgba(240,240,240,.14)', border: '1px solid rgba(240,240,240,.3)', color: 'var(--gold)', fontSize: '.75rem', letterSpacing: '.12em', textTransform: 'uppercase', padding: '.4rem 1rem', borderRadius: '50px', marginBottom: '1.5rem' }}>
            Analyse complète
          </div>
...
function ResultCard({ car, rank, isFirst, onDetail, onCompare }) {
  ...
  const formatPrice = p => p ? `${Math.round(p * 10.8).toLocaleString('fr-FR')} DH` : '—';
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
        {photo && (
          <div style={{ width: '160px', height: '110px', flexShrink: 0, borderRadius: '8px', overflow: 'hidden', background: 'var(--bg)' }}>
            <img src={photo} alt={`${car.make} ${car.model}`} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
          </div>
        )}
